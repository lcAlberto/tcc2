...
class AnimalHeatController extends Controller
{
    ...
    public function store(Request $request, AnimalHeat $animalHeat, AnimalServices $services, AnimalRepository $animalRepository)
    {
        $title = 'create-cio';
        $data = $request->all();
    ...
    public function edit($id)
    {
        ...
        if (isset($cios) && ($cios->farm_id == auth()->user()->farm_id)) {
            ...
            return view('animals.flock.cios.edit', compact('cios', 'animal', 'dateTime', 'title'));
        } else
            return redirect()->route('cio.index');
    }
                           ...
                           AnimalRepository $animalRepository, $id)
    {
        ...
        $data = $services->managementFather($request, $data);
        $data = $animalRepository->farm_by($data);
        $data = $services->partoSucesso($request, $data);
    ...
    public function show($id)
    {
        ...
        if (isset($cio) && ($cio->farm_id == auth()->user()->farm_id)) {
            ...
            if ($animal == null)
                return redirect()->route('cio.index');
            return view('animals.flock.cios.show', compact('cio', 'animal', 'title', 'animal'));
        } else return redirect()->route('cio.index');
    }
    ...
    public function heatByAnimal($id)
    {
        ...
        if (isset($cio) && ($cio->farm_id == auth()->user()->farm_id)) {
            ...
            return view('animals.flock.cios.index',
                compact('title', 'animals', 'cio'));
        } else
            return redirect()->route('cio.index');
    }

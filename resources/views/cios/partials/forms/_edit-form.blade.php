    ...
    <div class="col-lg-5 col-md-5 col-sm-12">
        <div class="ml-2 form-group">
            <div class="form-group mb-3">
                ...
                <select class="form-control" id="childbirth_type" name="childbirth_type" required>
                    <option {{old('childbirth_type') == 'natural' ? 'selected' : '' }} id="natural" value="natural">
                        Natural
                    </option>
                    <option {{old('childbirth_type') == 'insemination' ? 'selected' : ''}} id="insemination"
        ...
        <div class="form-group mb-3 {{$errors->has('status') ? 'text-danger border-danger is-invalid' : ''}}">
            ...
            <select
                ...
                <option value="abortion" {{old('status' == 'abortion' ? 'selected' : '')}}>Aborto</option>
                {{--                @if($cios->date_childbirth_foreseen < today())--}}
                <option value="success" {{old('status' == 'success' ? 'selected' : '')}}>Sucesso</option>
                {{--                @endif--}}
            </select>
            <small>
                De maneira foi a cobertura?
        ...
        <div class="form-group mb-3" id="current-bull">
            <fieldset class="p-2 bg-translucent-white border border-top border-light border-dashed">
                ...
                <select name="father" class="form-control mb-3" id="touro">
                    ...
                    <option value="{{$cios->father}}" {{old('father') == $cios->father ? 'selected' : ''}}>
                        {{ $cios->father }}
                    </option>
                    @if(isset($animal->father))
                        <option value="{{$animal->father}}" {{old('father') == $animal->father ? 'selected' : ''}}>
                            {{$animal->father}}
                        </option>
                    @endif
                </select>
                <button class="btn btn-white btn-block text-primary" type="button" id="add-input" onclick="showInsemination()">
